...
def export_all_svgs():
    ...
    output_dir = os.path.join(workspace_dir, 'APPENDIX H', 'svg')
    new_app_svg_dir = os.path.join(workspace_dir, 'NEW APPENDIX', 'svg')
    appendix_d_dir = os.path.join(workspace_dir, 'APPENDIX D')
    
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(new_app_svg_dir, exist_ok=True)
    ...
    def render_svg(
        ...
        mime_type = 'image/png' if src_filename.lower().endswith('.png') else 'image/jpeg'
        ...
        svg_lines = []
        svg_lines.append(f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 {canvas_w} {canvas_h}" width="{canvas_w}" height="{canvas_h}">')
        svg_lines.append(f'  <defs>')
        svg_lines.append(f'    <style>')
        svg_lines.append(f'      .callout-text {{ font-family: "Times New Roman", Times, serif; font-size: {font_size}px; fill: #000000; text-anchor: middle; }}')
        svg_lines.append(f'      .box-rect {{ fill: #FFFFFF; stroke: {RED_HEX}; stroke-width: {line_width}; }}')
        svg_lines.append(f'      .arrow-line {{ stroke: {RED_HEX}; stroke-width: {line_width}; fill: none; stroke-linecap: square; stroke-linejoin: miter; }}')
        svg_lines.append(f'      .arrow-head {{ fill: {RED_HEX}; stroke: none; }}')
        svg_lines.append(f'    </style>')
        svg_lines.append(f'  </defs>')
        
        # Background Layer
        svg_lines.append(f'  <!-- Background Canvas -->')
        svg_lines.append(f'  <rect id="Background" width="{canvas_w}" height="{canvas_h}" fill="#FFFFFF"/>')
        
        # Screenshot Image Layer
        svg_lines.append(f'  <!-- Screenshot -->')
        svg_lines.append(f'  <g id="Screenshot_Frame">')
        svg_lines.append(f'    <image id="Screenshot_Image" x="{img_x}" y="{img_y}" width="{orig_w}" height="{orig_h}" xlink:href="data:{mime_type};base64,{b64_str}"/>')
        svg_lines.append(f'    <rect id="Screenshot_Border" x="{img_x - 1}" y="{img_y - 1}" width="{orig_w + 2}" height="{orig_h + 2}" fill="none" stroke="#BEBEBE" stroke-width="1"/>')
        svg_lines.append(f'  </g>')
        
        # Callout Boxes Group
        svg_lines.append(f'  <!-- Callout Boxes -->')
        svg_lines.append(f'  <g id="Callout_Boxes">')
        ...
        for idx, c in enumerate(callouts):
            ...
            bw = col_w
            bh = box_height
            box_coords.append((bx, by, bw, bh))
            
            svg_lines.append(f'    <g id="Box_{idx+1}">')
            svg_lines.append(f'      <rect class="box-rect" x="{bx}" y="{by}" width="{bw}" height="{bh}" rx="0" ry="0"/>')
            
            lines = wrap_text(c['text'], font_callout, bw - int(20 * (orig_w / 1280.0 if orig_w > 2000 else 1.0)))
            line_height = int(font_size * 1.25)
            total_text_h = len(lines) * line_height
            ty_start = by + (bh - total_text_h) // 2 + int(font_size * 0.85)
            
            for l_idx, line in enumerate(lines):
                lx = bx + bw // 2
                ly = ty_start + l_idx * line_height
                safe_line = line.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')
                svg_lines.append(f'      <text class="callout-text" x="{lx}" y="{ly}">{safe_line}</text>')
            svg_lines.append(f'    </g>')
        svg_lines.append(f'  </g>')
        
        # Arrow Vectors Group
        svg_lines.append(f'  <!-- Arrow Vectors (Editable in Figma) -->')
        svg_lines.append(f'  <g id="Arrows">')
        
        for idx, c in enumerate(callouts):
            bx, by, bw, bh = box_coords[idx]
            ...
            a2 = (abs_tx - arrow_len * math.cos(arrow_angle + math.pi/6),
                  abs_ty - arrow_len * math.sin(arrow_angle + math.pi/6))
            poly_points = f"{abs_tx},{abs_ty} {a1[0]:.2f},{a1[1]:.2f} {a2[0]:.2f},{a2[1]:.2f}"
            
            svg_lines.append(f'    <g id="Arrow_{idx+1}">')
            svg_lines.append(f'      <path class="arrow-line" d="{path_d}"/>')
            svg_lines.append(f'      <polygon class="arrow-head" points="{poly_points}"/>')
            svg_lines.append(f'    </g>')
        ...
        new_app_out = os.path.join(new_app_svg_dir, out_filename)
        with open(new_app_out, 'w', encoding='utf-8') as f:
            f.write(full_svg_str)
            
        print(f"Generated SVG: {out_filename}")
    ...
    render_svg(
        '5 Game Example.PNG',
        'Fig-H6-Overworld-Exploration.svg',
        [
    ...
    render_svg(
        ...
        'Fig-H9-Character-Skills-Menu.svg',
        [
    ...
    render_svg(
        ...
        'Fig-H21-Math-Combat-Keypad.svg',
        [
    ...
    print('All 21 SVGs exported successfully with calibrated coordinates.')
