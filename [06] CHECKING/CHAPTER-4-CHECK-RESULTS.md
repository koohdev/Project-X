# 🔍 Writing Check Results for Chapter 4 — Methodology

> **File Checked:** `[COA]-CHAPTER-4.md`
> **Date:** February 27, 2026
> **Checker Reference:** `ACADEMIC_WRITING_CHECKER.md` (all 7 categories)
...
|---|---|---|---|---|---|
| 1 | 15 | "Additionally, the proponents surveyed..." | AI-STRUCT | Over-smooth transition — "Additionally" is an AI structural pattern | Remove the transition or vary: "The proponents also surveyed..." |
| 2 | 41 | "ensuring that all components work together" | BANNED | "ensuring" is a form of banned word "ensure" | "verifying that all components work together" or "confirming that all components work together" |
| 3 | 138 | "several risks" | VAGUE | Vague quantifier — "several" does not specify a count | "six risks" (count the actual risks listed in the paragraph) |
| 4 | 145 | "to ensure that mathematical problems remain readable" | BANNED | "ensure" is a banned word | "to confirm that mathematical problems remain readable" or "to verify that mathematical problems remain readable" |
| 5 | 158 | "with many languages" | VAGUE | Vague quantifier — "many" is uncited | Specify: "with support for multiple character sets" or remove the clause |
| 6 | 57 | "HTML5 and WebGL" | TECH | "WebGL" used without definition | "HTML5 and Web Graphics Library (WebGL)" |
| 7 | 57 | "haptic feedback" | TECH | Jargon without definition — panelists may not know this term | "the physical key-press sensation (haptic feedback) that touchscreens lack" |
| 8 | 22 | "RPG Maker MZ and JavaScript... PeerJS... P2P... ATB" | TECH-STACK | Buzzword stacking — five technical terms in one sentence | Split into two sentences: one for the Math Battle System, one for multiplayer and ATB |
| 9 | 25 | "the proponents Capstone Adviser" | GRAMMAR | Missing possessive apostrophe | "the proponents' Capstone Adviser" |
| 10 | 140 | "the proponents Capstone Adviser" | GRAMMAR | Missing possessive apostrophe | "the proponents' Capstone Adviser" |
| 11 | 140 | "students computers" | GRAMMAR | Missing possessive apostrophe | "students' computers" |
| 12 | 53 | "exercises, however, a common limitation" | GRAMMAR | Comma splice — two independent clauses joined with a comma | "exercises; however, a common limitation..." or split into two sentences |
...
| 🔬 Technical Issues | 3 |
| 📋 Chapter Relevance Issues | 0 |
| 📌 Title Drift Issues | 0 |
| 🚫 Banned Words | 2 |
| ✏️ Grammar Notes | 4 |
| **TOTAL** | **12** |
...
Below is the full text of Chapter 4 with **all fixes applied**. Changes are marked with `←` comments for reference.
...
Existing educational game platforms such as Prodigy Math and Math Blaster address arithmetic instruction through gamified exercises; however, a common limitation is that mathematical tasks function as a separate reward layer rather than directly determining gameplay outcomes. ← *Fixed comma splice with semicolon* Combat sequences in these tools proceed regardless of input accuracy or response speed, which the proposed title is designed to address.
...
The proposed title deploys as a Windows desktop application via NW.js (Node Webkit) and as a web application hosted on Hostinger using HTML5 and Web Graphics Library (WebGL). ← *Expanded "WebGL"* Desktop is the primary platform because the Math Battle System requires timed numerical input through a physical keyboard, which touchscreens cannot replicate due to the absence of a physical key-press sensation (haptic feedback). ← *Defined "haptic feedback"* The desktop executable can also operate offline using a computer. The web deployment serves as a secondary access point, supported by the Virtual Numeric Keypad module for touch-enabled devices.
...
The proponents chose the M+ 1m regular font as it is a typeface made for clear reading with uses that range from digital signs and multilingual systems, to computer screens and tools for writing code. ← *Replaced "systems with many languages" with "multilingual systems" to remove vague quantifier*
